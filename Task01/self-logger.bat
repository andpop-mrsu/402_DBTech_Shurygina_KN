...
for /f %%c in ('%SQLITE_PATH% %DB_NAME% "SELECT COUNT(*) FROM logs;"') do set count=%%c
echo Количество запусков: %count%

for /f "tokens=*" %%a in ('%SQLITE_PATH% %DB_NAME% "SELECT MIN(log_time) FROM logs ORDER BY log_time ASC LIMIT 1;"') do set first_log=%%a
...
%SQLITE_PATH% %DB_NAME% "SELECT username || ' | ' || log_time FROM logs ORDER BY log_time;"
echo ---------------------------------------------
